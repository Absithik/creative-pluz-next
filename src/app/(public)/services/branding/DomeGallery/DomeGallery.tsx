...
function buildItems(pool: (string | ImageData)[], seg: number) {
    ...
    const normalizedImages: ImageData[] = pool.map(image => {
...
export default function DomeGallery({
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver(entries => {
            ...
            root.style.setProperty('--image-filter', grayscale ? 'grayscale(0)' : 'none');
        ...
        (el: HTMLElement) => {
            ...
            if (wantsResize) {
                const onFirstEnd = (ev: TransitionEvent) => {
                    ...
                    const cleanupSecond = () => {
                        overlay.removeEventListener('transitionend', cleanupSecond as any);
                        overlay.style.transition = prevTransition;
                    };
                    overlay.addEventListener('transitionend', cleanupSecond as any, { once: true });
                };
